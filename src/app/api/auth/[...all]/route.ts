//for arcject protection use authclient logic instead of server auth logic
...
async function protect(req: NextRequest): Promise<ArcjetDecision> {
  ...
  if (req.nextUrl.pathname.endsWith("api/auth/sign-up")) {
    ...
    if (typeof body.email === "string") {
      return aj
        ...
        .protect(req, { email: body.email, userIdOrIp: userId });
    } else {
      // Otherwise use rate limit and detect bot
      return aj
        .withRule(detectBot(botSettings))
        .withRule(slidingWindow(restrictiveRateLimitSettings))
        .protect(req, { userIdOrIp: userId });
    }
  } else {
