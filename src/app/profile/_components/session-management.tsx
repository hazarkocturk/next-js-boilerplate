...
const revokeOtherSessions = async () => {
  const res = await authClient.revokeOtherSessions(undefined, {
    onSuccess: () => {
      router.refresh()
    },
  })
  ...
  if (res && "error" in res && res.error) {
    return {
      error: res.error.message ?? "Failed to revoke other sessions",
    }
  }
...
function SessionCard({
 ...
 const revokeSession = async () => {
  ...
  if (res && "error" in res && res.error) {
    return {
      error: res.error.message ?? "Failed to revoke session",
    }
  }
